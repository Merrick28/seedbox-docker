...
for myvar in BASE_URL ADMIN_URL PASSWD_FILE CF_API_EMAIL CF_API_KEY\
 CONFIG_DIR DATA_DIR PORTAINER_URL RUTORRENT_BASE_URL MEDUSA_BASE_URL \
 ...
 NEXTCLOUD_BASE_URL PLEX_URL SABNZBD_BASE_URL PYLOAD_BASE_URL FLARESOLVERR_BASE_URL JELLYFIN_URL TZ LVM_STATUS LVM_VG_NAME LVM_MAPPER
do
    if [ -z "${!myvar}" ]; then
